...
exports.getBookings = async (req, res) => {
	try {
		...
		if (roomId) filter.room = roomId;
		if (date) filter.date = parseISO(date);
		if (userId) filter.user = userId;
...
exports.cancelBooking = async (req, res) => {
	try {
		...
		if (booking.user.toString() !== req.user.id && req.user.role !== "admin") {
			return res.status(403).json({ message: "Tidak ada izin" });
		}

		// minimal 2 jam sebelum mulai
		const bookingDateTime = new Date(booking.date);
		const [hours, minutes] = booking.startTime.split(":").map(Number);
		bookingDateTime.setHours(hours, minutes, 0, 0);
		...
		if (hoursUntilBooking < 2 && req.user.role !== "admin") {
			return res.status(400).json({
				message: "Booking hanya dapat dibatalkan minimal 2 jam sebelum mulai",
			});
		}
...
exports.bookRoom = async (req, res) => {
	try {
		...
		// Required fields
		if (!date || !startTime || !endTime || !phone) {
			return res.status(400).json({ message: "date, startTime, endTime, dan phone wajib diisi" });
		}

		// Validate phone
		if (!validatePhoneNumber(phone)) {
			return res.status(400).json({ message: "Format nomor telepon tidak valid. Gunakan format 08xxxxxxxx" });
		}
		...
		if (d < today) return res.status(400).json({ message: "Tanggal sudah lewat" });

		// Only weekdays
		if (!isWorkingDay(d)) {
			return res.status(400).json({ message: "Peminjaman hanya Senin-Jumat" });
		}

		// Validate time format
		const timeRegex = /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/;
		if (!timeRegex.test(startTime) || !timeRegex.test(endTime)) {
			return res.status(400).json({ message: "Format waktu tidak valid (HH:MM)" });
		}

		// Duration
		const duration = calculateDurationHours(startTime, endTime);
		if (duration < 1) return res.status(400).json({ message: "Durasi minimal 1 jam" });

		// Operating hours
		if (!isWithinOperatingHours(startTime, endTime)) {
			return res.status(400).json({ message: "Jam operasional: 08:00 - 17:00" });
		}
		...
		res.status(201).json({ message: "Ruangan berhasil dipesan", booking: populatedBooking });
	} catch (err) {
		console.error(err);
		if (err.name === "ValidationError") {
			return res.status(400).json({
